    ...
    <div className="min-h-screen bg-[#060611] text-white overflow-x-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-bold gradient-text-purple">AI Workspace</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/register">
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="bg-gradient-animated text-white text-sm font-medium px-4 py-2 rounded-lg animate-pulse-glow"
              >
                Get started free
              </motion.button>
            </Link>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 text-center max-w-5xl mx-auto px-6">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass border border-violet-500/20 text-violet-300 text-sm font-medium mb-8"
          >
            <span className="w-2 h-2 rounded-full bg-violet-400 animate-pulse" />
            Powered by Groq Llama 3 70B — 500 tokens/sec
            <ArrowRight size={14} />
          </motion.div>
          ...
          >
            <Link to="/register">
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                className="bg-gradient-animated text-white font-semibold px-8 py-4 rounded-xl text-lg flex items-center gap-2 bg-size-300"
              >
                Start building free
                <ArrowRight size={20} />
              </motion.button>
            ...
            <Link to="/login">
              <motion.button
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                className="glass border border-white/[0.1] text-white/80 hover:text-white font-medium px-8 py-4 rounded-xl text-lg flex items-center gap-2 transition-colors"
              >
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <FadeIn>
            <div className="text-center mb-20">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400 text-sm mb-4">
                <Layers size={14} />
                Everything you need
              </div>
            ...
            {features.map(({ icon: Icon, title, desc, color }, i) => (
              <FadeIn key={title} delay={i * 0.08}>
                <motion.div
                  ...
                  className="glass-premium rounded-2xl p-6 cursor-default group shimmer-effect holographic"
                >
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 glow-purple`}>
            ...
            {steps.map(({ step, title, desc }, i) => (
              <FadeIn key={step} delay={i * 0.1}>
                <div className="text-center">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-500/20 to-blue-500/20 border border-violet-500/20 flex items-center justify-center mx-auto mb-4">
                    <span className="font-display font-bold text-violet-400 text-lg">{step}</span>
                  </div>
                  <h3 className="font-display font-semibold text-white mb-2">{title}</h3>
                  <p className="text-white/50 text-sm leading-relaxed">{desc}</p>
                </div>
                ...
                >
                  {popular && (
                    <motion.div
                      ...
                      className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-violet-500 to-blue-500 text-white text-xs font-bold px-4 py-1 rounded-full glow-purple"
                    >
                      Most Popular
                    </motion.div>
                  )}
                  <div className="mb-6">
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="font-display text-4xl font-bold gradient-text">{price}</span>
                  ...
                  <Link to="/register">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`w-full py-3 rounded-xl font-medium text-sm transition-all ${popular
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map(({ name, role, avatar, text, rating }, i) => (
              <FadeIn key={name} delay={i * 0.1}>
                <motion.div whileHover={{ y: -4 }} className="glass-card rounded-2xl p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-400 to-blue-400 flex items-center justify-center text-white text-xs font-bold">
                      {avatar}
                    </div>
                    <div>
          ...
          <div className="max-w-3xl mx-auto text-center">
            <div className="glass-card rounded-3xl p-12 neon-border relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-500 to-blue-500 flex items-center justify-center mx-auto mb-6 glow-purple">
                ...
                <Link to="/register">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                    className="bg-gradient-animated text-white font-semibold px-10 py-4 rounded-xl text-lg flex items-center gap-3 mx-auto"
                  >
                    Start for free
                    <ArrowRight size={20} />
                  </motion.button>
      ...
      <footer className="py-12 px-6 border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-white/30 text-sm">© 2025 AI Workspace. All rights reserved.</p>
